    ...
    return (
        <div className={'w-screen h-screen overflow-hidden'}>
            ...
            <div className={'w-full h-full flex'}>
                ...
                <div className={'w-1/2 h-full shadow-xl flex flex-col'}>
                    <h1>Sub Total : ${cart.length > 0 ? cart.reduce((price, curr) => price + curr.price, 0) : '0.00'}</h1>
                    <button onClick={makePayment} className={'bg-[#f04037] px-5 py-3 rounded-xl'}>Pay</button>
                </div>
            </div>
        </div>
    );
